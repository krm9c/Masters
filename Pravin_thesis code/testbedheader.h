#include <stdio.h>
#include "linux/arcom/aim104.h" // library supporting ADC functions
#include <math.h>
#include "rtai.h"
...
#define inp(x)          inb(x)  //had defined based on borland c code which is reverse to that
#define	outp(x,y)	      outb(y,x) //of linux gcc code...
...
#define LoadILBit           0x01 // command to load integral limit for PID filter

#define LoadAllFil          (LoadKPBit | LoadKIBit | LoadKDBit | LoadILBit)

#define TICKPERIOD 	1000000  // macro defining tickperiod to be 1ms (1000000 nanoseconds)
...
static long int current_state;/*global variable defining current state */
...
void* truck_potentiometer(void* filehandle4);//updates trailer cab angle

void* truck_steering(void* filehandle5);//updates steering angle


void* truck_accelerometer_x(void* filehandle1);// updates truck acceleration truck_accel_x along x axis

void* truck_accelerometer_y(void* filehandle2);// updates truck acceleration truck_accel_y along y axis

void* truck_accelerometer_z(void* filehandle3);// updates truck acceleration truck_accel_z along z axis
